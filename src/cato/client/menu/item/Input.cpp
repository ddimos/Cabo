    ...
    , m_maxSize(_maxSize)
{
    ...
    m_textRef.setPosition(Position{ 
        .m_position = sf::Vector2f(10.f, -12.f), // TODO looks like it doesn't properly calculates the parent pos, so I have to adjust it like this
...
void Input::onRegisterEvents(core::event::Dispatcher& _dispatcher, bool _isBeingRegistered)
{
    if (_isBeingRegistered)
    {
        ...
        _dispatcher.registerEvent<events::TextEnteredEvent>(m_listenerId,
            [this](const events::TextEnteredEvent& _event){
                ...
                processChar(_event.text.unicode);
            }
        );
        _dispatcher.registerEvent<events::TextPastedEvent>(m_listenerId,
            [this](const events::TextPastedEvent& _event){
                if (!m_isInInputMode)
                    return;

                size_t i = m_enteredStr.size();
                for (char c : _event.text)
                {
                    if (i >= m_maxSize)
                        break;
                    processChar(c);
    ...
    else
    {
        ...
        _dispatcher.unregisterEvent<events::TextPastedEvent>(m_listenerId);
    }
}
...
void Input::processChar(uint32_t _char)
{
    if (_char == 8) // Backspace
        return;
    
    if (_char == 22) // The symbol is generated by pressing ctrl+V
        return;

    if(m_enteredStr.size() < m_maxSize)
    {
        m_enteredStr += _char;    // To be able to validate the full string
        if (!m_onValidateEnteredTextCallback(_char, m_enteredStr))
            m_enteredStr.pop_back();
